...
class FATAnalyzer:
    ...
    def read_cluster_data(self, n, size):
        if n < 2: return b""
    ...
    def get_fat_chain(self, start_cluster):
        chain = []
        curr = start_cluster
        limit = 0xFFF8 if self.fat_type == "FAT16" else 0xFF8
        while 2 <= curr < limit and len(chain) < 10000:
            chain.append(curr)
            next_clus = self.get_fat_val(curr)
            if next_clus == curr: break 
            curr = next_clus
        return chain

    def scan_recursive(self, offset, count_limit, parent_path, is_root):
        self.f.seek(offset)
        read_size = count_limit * 32 if is_root else self.clus_sz
        ...
        for i in range(0, len(raw), 32):
            ...
            fst = struct.unpack('<H', rec[26:28])[0]
            ...
            if attr & 0x10:
                ...
                if fst >= 2:
                    sub_off = self.addr_data + (fst - 2) * self.clus_sz
                    curr_pos = self.f.tell()
                    self.scan_recursive(sub_off, 65536, parent_path + full_name + "/", False)
    ...
    def print_full_report(self):
        ...
        for item in self.tree_items:
            ...
            print(f"ОБЪЕКТ: {name} ({'ПАПКА' if item.get('is_dir') or 'D' in item['attr'] else 'ФАЙЛ'})")
